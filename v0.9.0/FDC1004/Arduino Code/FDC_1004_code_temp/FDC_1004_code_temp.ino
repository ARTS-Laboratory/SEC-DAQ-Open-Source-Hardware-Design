...
//////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
int CapOffset = 50;     // Cap offset in Pf. Max of 96 pF.                  //////////
...
// PIN adresses the DS1631 temp sensor, all address headers pulled low.
#define DS1631_ADDR 0x90 >> 1

// SD card
const int chipSelect = 4;                  // read only, chip select set to 4
...
Serial.print("Initializing SD card...");
  ...
  pinMode(10, OUTPUT);                   // chip select pin is set to output

   if (!SD.begin(chipSelect)) {          // see if the card is present and can be initialized:
   Serial.println("Card failed, or not present");
   return;
  }
  Serial.println("card initialized.");   // prints to data file if card is present  
   // save headers to SD card //////////////////////////////////////////////////////////////////////////////////////////////// 

  File dataFile = SD.open("datalog.txt", FILE_WRITE);   // creates and opens a file called datalog.txt the file

  if (dataFile) {                                       // if the file is available, write to it:
    //dataFile.println("%  ...");                            // headers for ID of the probram, columun headers and units.  
    //dataFile.println("%  Data collection, DC resistance");
    //dataFile.println("%  Mark 1, SN 1002, 2015 Austin Downey");
   //dataFile.println("%  austindowney@gmail.com, adowney2.public.iastate.edu");
    //dataFile.println("%  Resistor value = reference resistor*((5/sensor_#_volts)-1) ");
    dataFile.println("%  Temperature (C), Sensor_1 (Volts),Sensor_2 (Volts),Sensor_3 (Volts), Time (s) ");
    Serial.print("Temperature (C), Sensor_1 (Volts),Sensor_2 (Volts),Sensor_3 (Volts), Time (s)");
    //dataFile.println("%  ...");                            // used to create a clean line break
...
void loop()
{
  ...
  // collect the temperature and prefom the limited math functions needed
 ...
 Th = Th - 256;
 int T_dec=(10*(100*(Tl/16)))/16; // decimal part of the TÂ°
